...
function html_showgallerys( $rows,  $pageNav,$sort,$cat_row){
	global $wpdb;
        $wp_nonce = wp_create_nonce('huge_it_gallery_check');
	?>
	...
	<div id="poststuff">
		<div id="gallerys-list-page">
			...
			<h2><?php echo __('Huge-IT Galleries', 'gallery-images'); ?>
				<a onclick="window.location.href='admin.php?page=gallerys_huge_it_gallery&task=add_cat&huge_it_nonce=<?php echo $wp_nonce;?>'" class="add-new-h2" ><?php echo __('Add New Gallery', 'gallery-images'); ?></a>
			</h2>
			<?php
			$serch_value='';
				 ...
				 $trcount=1;
				  for($i=0; $i<count($rows);$i++){
					...
					<tr <?php if($trcount%2==0){ echo 'class="has-background"';}?>>
						...
						<td><a  href="admin.php?page=gallerys_huge_it_gallery&task=edit_cat&id=<?php echo $rows[$i]->id?>&huge_it_nonce=<?php echo $wp_nonce;?>"><?php echo esc_html(stripslashes($rows[$i]->name)); ?></a></td>
...
$wp_nonce = wp_create_nonce('huge_it_gallery_check');
	...
	if($_GET["addslide"] == 1){
	header('Location: admin.php?page=gallerys_huge_it_gallery&id='.$row->id.'&task=applyadd_cat&huge_it_nonce='.$wp_nonce);
	}
	}
?>
...
function submitbutton(pressbutton) 
{
	...
	filterInputs();		
	document.getElementById("adminForm").action=document.getElementById("adminForm").action+"&task="+pressbutton+"&huge_it_nonce=<?php echo $wp_nonce;?>";
	...
	<div id="gallery-header">
		<ul id="gallerys-list">
			...
			foreach($rowsld as $rowsldires){
				...
				?>
					<li>
						<a href="#" onclick="window.location.href='admin.php?page=gallerys_huge_it_gallery&task=edit_cat&id=<?php echo $rowsldires->id; ?>task=add_cat&huge_it_nonce=<?php echo $wp_nonce;?>'" ><?php echo $rowsldires->name; ?></a>
					</li>
				<?php
				}
				else{ ?>
		...
		?>
			<li class="add-new">
				<a onclick="window.location.href='admin.php?page=gallerys_huge_it_gallery&amp;task=add_cat&huge_it_nonce=<?php echo $wp_nonce;?>'">+</a>
			</li>
		</ul>
		</div>
