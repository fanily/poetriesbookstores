...
function editgallery($id)
  {
        global $wpdb;
        $wp_nonce = $_GET['huge_it_nonce'];
        if(!wp_verify_nonce($wp_nonce, 'huge_it_gallery_check')) {
...
( 'New gallery', '375', '600', 'on', 'cubeH', '4000', '1000', 'center', '1', '300', '4')";
   ...
   $query="SELECT * FROM ".$wpdb->prefix."huge_itgallery_gallerys order by id ASC";
	...
	foreach($rowsldcc as $key=>$rowsldccs){
		if($last_key == $key){
                        $wp_nonce = $_GET['huge_it_nonce'];
			header('Location: admin.php?page=gallerys_huge_it_gallery&id='.$rowsldccs->id.'&task=apply&huge_it_nonce='.$wp_nonce);
		}
	}	
}
...
$wpdb->query($wpdb->prepare("UPDATE ".$wpdb->prefix."huge_itgallery_images SET ordering = %d WHERE id = %d ", $rowplusorderspl,$rowplusorders->id));
}
...
//		if(!check_admin_referer('huge_it_gallery', 'huge_it_gallery_check')) {
//                    wp_die('Security check fail'); 
//                }
	...
	if(isset($_POST['changedvalues']) && $_POST['changedvalues'] != '') {
			...
			$chagedValues = preg_replace('#[^0-9,]+#','',$_POST['changedvalues']);	
